...
class BlockMonitor:
    def __init__(self, settings, db, handler, rpc_limiter=None):
    ...
    async def _call(self,awaitable):return await self.rpc_limiter.call(awaitable) if self.rpc_limiter else await awaitable
    async def run(self):
        delay=1.0
        while True:
            try:
    ...
    async def _range(self,start,end):
        ...
        for version, addresses, module in specs:
            ...
            logs=await self._call(self.w3.eth.get_logs({"fromBlock":start,"toBlock":end,"address":list(addresses),"topics":[module.TOPIC]}))
            for event in sorted(logs,key=lambda x:(x["blockNumber"],x["logIndex"])):
                token0,token1,pool,related=module.decode_log(event)
                tx=await self._call(self.w3.eth.get_transaction(event["transactionHash"]))
                block=await self._call(self.w3.eth.get_block(event["blockNumber"]))
                await self.handler({"address":pool,"block_number":event["blockNumber"],"timestamp":block["timestamp"],"tx_hash":event["transactionHash"].hex(),"dex_version":version,"token0":token0,"token1":token1,"creator":tx["from"].lower(),"factory":event["address"].lower(),"related":related,"event_topic":module.TOPIC},self.w3)
